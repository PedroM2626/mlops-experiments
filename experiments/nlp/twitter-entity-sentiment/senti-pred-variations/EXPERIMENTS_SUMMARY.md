...
## 7. Comparação rigorosa A vs B vs C (what-ifs)
- Ver `experiments/nlp/pipelines_abc_comparison/README.md`: reprodução canônica das três
  pipelines e ablações controladas de n-gramas, vocabulário, pré-processamento e modelo.
- Principais achados: remover bigramas derruba −2.6 a −4.8 pp; reduzir a C de 4-gramas para
  bigramas melhora +0.33 pp; vetorizador da C (100k) é o melhor componente, e
  pré-processamento A + vetorizador C alcança 0.9857 F1-Macro; diferenças < 1 pp entre
  pipelines não são significativas (McNemar).
