...
# Baixar recursos do NLTK no nível do módulo para evitar downloads repetidos em processos paralelos
...
def process_text_chunk(df_chunk):
    """Processa um pedaço do dataframe."""
...
def run_preprocessing():
    ...
    # Divide os dataframes em pedaços para processamento paralelo
    chunks_train = np.array_split(df_train, num_cores)
    chunks_val = np.array_split(df_val, num_cores) if not df_val.empty else []

    # Processamento paralelo para treino
    ...
    # Processamento paralelo para validação
    if not df_val.empty:
